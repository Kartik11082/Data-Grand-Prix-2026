...
def validate_composition(data: list[dict]) -> bool:
    ...
    for row in data:
        ...
        if total != 100.0:
            print(f"❌ Year {row['year']} mathematical split failed 100% check (sums to {total}%).")
            all_sum_100 = False
            
    if all_sum_100:
        print("✅ All years correctly split into cleanly rounded percentages totaling 100.0%.")
    else:
        is_valid = False
        
    yr2007 = next((d for d in data if d["year"] == 2007), None)
    if yr2007:
        print(f"📊 Tracking 2007 baseline for Government-Backed loans... Detected: {yr2007['govt_backed']}%")
        if abs(yr2007["govt_backed"] - 12.0) <= 5.0:  # Allow margin since data fluctuates natively
...
def plot_composition(data: list[dict], output_img_path: str) -> None:
    ...
    plt.stackplot(
        ...
        colors=['#3b82f6', '#10b981'],  # Tailored blues/greens matching explicit requests
        alpha=0.85
    )
    
    # Horizontal dash delineating exactly 12% 
    # Frontend explicitly maps <ReferenceLine y={0.12}/>
    plt.axhline(y=12, color='white', linestyle='--', linewidth=1.5, alpha=0.9, label='12% Govt-Backed Base (2007)')
...
def main() -> None:
    ...
    with open(mock_file, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["as_of_year", "action_taken", "loan_type", "is_originated", "is_conventional", "is_govt_backed"])
    ...
    master_df = spark.read.csv(f"file:///{mock_file.replace(os.path.sep, '/')}", header=True, inferSchema=True)
